...
var resizePhoto = function (photo, sizeType, layoutWidth, layoutHeight, order) { return __awaiter(void 0, void 0, void 0, function () {
    ...
    return __generator(this, function (_b) {
        ...
        updatedWidth = 0;
        updatedHeight = 0;
        _a = constants_1.PADDINGS[sizeType], xPadding = _a.xPadding, yPadding = _a.yPadding, innerPadding = _a.innerPadding;
        doubleYPadding = yPadding * 2;
        doubleXPadding = xPadding * 2;
        switch (sizeType) {
            case types_1.SIZE_TYPE.HALF: {
                updatedWidth = layoutWidth.getHalf();
                updatedHeight = layoutHeight;
                resizedPhoto = photo.resize(updatedWidth, updatedHeight).sharpen();
                break;
            }
            case types_1.SIZE_TYPE.THREE_QUARTERS: {
                if (innerPadding) {
                    updatedWidth = Math.round((layoutWidth - (constants_1.CUT_OFF * 2) - xPadding - innerPadding) * 0.80 + constants_1.CUT_OFF);
                }
                else {
            ...
            case types_1.SIZE_TYPE.HALF_CUTTED: {
            ...
            case types_1.SIZE_TYPE.THREE_HORISONTAL_HALF: {
            ...
            case types_1.SIZE_TYPE.FOUR_HORISONTAL_FULL: {
            ...
            case types_1.SIZE_TYPE.TWO_VERTICAL_ONE_HORISONTAL_HALF: {
                if (order === 1 || order === 2) {
                    if (innerPadding) {
            ...
            case types_1.SIZE_TYPE.TWO_HORISONTAL_HALF: {
                updatedWidth = Math.round(layoutWidth.getHalf().minusMargin()) - doubleXPadding;
                if (innerPadding) {
                    updatedHeight = Math.round((layoutHeight.minusMargins() - (doubleYPadding) - innerPadding) / 2);
                }
            ...
            case types_1.SIZE_TYPE.FOUR_VERTICAL_HALF: {
                if (innerPadding) {
                    updatedWidth = Math.round((layoutWidth.getHalf().minusMargin() - doubleXPadding - innerPadding) / 2);
                    updatedHeight = Math.round((layoutHeight.minusMargins() - (doubleYPadding) - innerPadding) / 2);
                }
            ...
            case types_1.SIZE_TYPE.TWO_VERTICAL_CUSTOM: {
                if (innerPadding) {
                    updatedWidth = Math.round((layoutWidth - (constants_1.CUT_OFF * 2) - xPadding - innerPadding) * 0.20);
                    updatedHeight = Math.round((layoutHeight.minusMargins() - (doubleYPadding) - innerPadding) / 2);
                }
                else {
                    console.log('innerPadding in not defined!');
